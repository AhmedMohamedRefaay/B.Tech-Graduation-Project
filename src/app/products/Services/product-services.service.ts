...
export class ProductServicesService {
  ...
  GetProductByCatId(categoryId:number):Observable<Iproduct[]>{
    ...
    return this.httpClient.get<Iproduct[]>(`http://localhost:5011/api/Product/GetAllProduct?categoryId=${categoryId}`);
  }
  GetAllProducts():Observable<Iproduct[]>{
    return this.httpClient.get<Iproduct[]>("http://localhost:5011/api/Product/GetAllProduct")

  }

  GetProductsDetailsByID(id:number):Observable<any>{
   // return this.httpClient.get<Iproduct>(`http://localhost:3000/products/${id}`);
   return this.httpClient.get<any>(`http://localhost:5011/api/Product/GetProductyById/${id}`);
  }
