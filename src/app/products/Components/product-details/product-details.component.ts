...
console.log(review);
var username = localStorage.getItem('currentUserUseName');
this.reviewService.add(productId,review).subscribe(() => {
 ...
 this.reviewForm.reset();
  this.reviewService.getAll(productId).subscribe((reviews) => {
    ...
    // if(username !=null)
    //  this.reviews[0].username=username;
    console.log(this.reviews);
...
AddProduct()
{
  ...
  var userid = localStorage.getItem('currentUserId');
    if(userid != null){
    this.http.post<any>('http://localhost:5011/api/WishList/AddProduct',{"UserID":userid,"productId":this.product.id}).subscribe(data=>{
    })
